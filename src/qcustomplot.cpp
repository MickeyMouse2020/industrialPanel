...
/*!
  ...
  \see columnCount
*/
int QCPLayoutGrid::m_rowCount() const
{
...
{
  if (row >= 0 && row < m_rowCount() && column >= 0 && column < columnCount())
...
/*!
  Sets the stretch \a factors of all rows. \a factors must have the size \ref m_rowCount.
...
/*!
  ...
  range from 0 (inserts a row at the top) to \a m_rowCount (appends a row at the bottom).
...
/*!
  ...
  newIndex range from 0 (inserts a row at the left) to \a m_rowCount (appends a row at the right).
...
void QCPLayoutGrid::updateLayout()
{
  ...
  int totalRowSpacing = (m_rowCount()-1) * mRowSpacing;
...
QSize QCPLayoutGrid::minimumSizeHint() const
{
  ...
  result.rheight() += qMax(0, m_rowCount()-1) * mRowSpacing + mMargins.top() + mMargins.bottom();
  return result;
}
...
void QCPLayoutGrid::getMinimumRowColSizes(QVector<int> *minColWidths, QVector<int> *minRowHeights) const
{
  ...
  *minRowHeights = QVector<int>(m_rowCount(), 0);
  for (int row=0; row<m_rowCount(); ++row)
  {
    for (int col=0; col<columnCount(); ++col)
    {
...
void QCPLayoutGrid::getMaximumRowColSizes(QVector<int> *maxColWidths, QVector<int> *maxRowHeights) const
{
  ...
  *maxRowHeights = QVector<int>(m_rowCount(), QWIDGETSIZE_MAX);
  for (int row=0; row<m_rowCount(); ++row)
  {
    for (int col=0; col<columnCount(); ++col)
    {
...
void QCPAbstractPlottable::rescaleValueAxis(bool onlyEnlarge) const
{
  ...
  if (!valueAxis) { qDebug() << Q_FUNC_INFO << "invalid m_value axis"; return; }
...
void QCPAbstractPlottable::coordsToPixels(double key, double value, double &x, double &y) const
{
  QCPAxis *keyAxis = mKeyAxis.data();
  QCPAxis *valueAxis = mValueAxis.data();
  if (!keyAxis || !valueAxis) { qDebug() << Q_FUNC_INFO << "invalid key or m_value axis"; return; }
...
const QPointF QCPAbstractPlottable::coordsToPixels(double key, double value) const
{
  QCPAxis *keyAxis = mKeyAxis.data();
  QCPAxis *valueAxis = mValueAxis.data();
  if (!keyAxis || !valueAxis) { qDebug() << Q_FUNC_INFO << "invalid key or m_value axis"; return QPointF(); }
...
bool QCPLegend::addItem(QCPAbstractLegendItem *item)
{
  if (!hasItem(item))
  {
    return addElement(m_rowCount(), 0, item);
  } else
    return false;
}
...
double QCPGraph::selectTest(const QPointF &pos, bool onlySelectable, QVariant *details) const
{
  ...
  if ((onlySelectable && !mSelectable) || mData->isEmpty())
    return -1;
  if (!mKeyAxis || !mValueAxis) { qDebug() << Q_FUNC_INFO << "invalid key or m_value axis"; return -1; }
...
{
  if (!mKeyAxis || !mValueAxis) { qDebug() << Q_FUNC_INFO << "invalid key or m_value axis"; return; }
  ...
  {
    if (QCP::isInvalidData(it.m_value().key, it.m_value().m_value) ||
        QCP::isInvalidData(it.m_value().keyErrorPlus, it.m_value().keyErrorMinus) ||
        QCP::isInvalidData(it.m_value().valueErrorPlus, it.m_value().valueErrorPlus))
...
const QPolygonF QCPGraph::getChannelFillPolygon(const QVector<QPointF> *lineData) const
{
  ...
  QCPAxis *keyAxis = mKeyAxis.data();
  QCPAxis *valueAxis = mValueAxis.data();
  if (!keyAxis || !valueAxis) { qDebug() << Q_FUNC_INFO << "invalid key or m_value axis"; return QPolygonF(); }
...
void QCPCurve::draw(QCPPainter *painter)
{
  ...
  for (it = mData->constBegin(); it != mData->constEnd(); ++it)
  {
    if (QCP::isInvalidData(it.m_value().t) ||
        QCP::isInvalidData(it.m_value().key, it.m_value().m_value))
      qDebug() << Q_FUNC_INFO << "Data point at" << it.key() << "invalid." << "Plottable name:" << name();
  }
#endif
...
void QCPBars::moveBelow(QCPBars *bars)
{
  ...
  {
    qDebug() << Q_FUNC_INFO << "passed QCPBars* doesn't have same key and m_value axis as this QCPBars";
    return;
  }
...
void QCPColorMap::updateMapImage()
{
  ...
  if (keyAxis->orientation() == Qt::Horizontal)
  {
    ...
    for (int line=0; line<lineCount; ++line)
    {
      ...
      mGradient.colorize(rawData+line*m_rowCount, mDataRange, pixels, m_rowCount, 1, mDataScaleType==QCPAxis::stLogarithmic);
    }
  } else // keyAxis->orientation() == Qt::Vertical
  {
    const int lineCount = keySize;
    const int m_rowCount = valueSize;
    for (int line=0; line<lineCount; ++line)
    {
      QRgb* pixels = reinterpret_cast<QRgb*>(localMapImage->scanLine(lineCount-1-line)); // invert scanline index because QImage counts scanlines from top, but our vertical index counts from bottom (mathematical coordinate system)
      mGradient.colorize(rawData+line, mDataRange, pixels, m_rowCount, lineCount, mDataScaleType==QCPAxis::stLogarithmic);
    }
  }
...
double QCPFinancial::ohlcSelectTest(const QPointF &pos, const QCPFinancialDataMap::const_iterator &begin, const QCPFinancialDataMap::const_iterator &end) const
{
  QCPAxis *keyAxis = mKeyAxis.data();
  QCPAxis *valueAxis = mValueAxis.data();
  if (!keyAxis || !valueAxis) { qDebug() << Q_FUNC_INFO << "invalid key or m_value axis"; return -1; }
